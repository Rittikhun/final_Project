...
class CalendarView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    lazy var gregorian : Calendar = {
        ...
        cal.timeZone = TimeZone(abbreviation: "UTC+07")!
    ...
    var eventsByIndexPath : [IndexPath:[CalendarEvent]] = [IndexPath:[CalendarEvent]]()
    ...
    var events : [EKEvent]? {
        ...
        didSet {
            ...
            for event in events {
                ...
                let calEvent = CalendarEvent(title: event.title, startDate: startDate, endDate: endDate, location: event.location!)
                ...
                if var eventsList : [CalendarEvent] = eventsByIndexPath[indexPath] { // If we have initialized a list for this IndexPath
                    ...
                    eventsByIndexPath[indexPath]?.append(calEvent)
    ...
    override var frame: CGRect {
        didSet {
            ...
            self.headerView.frame   = CGRect(x:0.0, y:10.0, width: frame.size.width, height:HEADER_DEFAULT_HEIGHT)
